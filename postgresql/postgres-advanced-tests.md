## 25.10 2025 HIT RATE = 50% SCORE = 11 из 15 - НЕЗАЧЕТ =(
  комментарий - после тестирования - прогнал чз gpt-5 - поправил 2 ошибки

### 1. Существует структура базы данных. Какая команда на месте [...] позволит создать новую запись в таблице role?
...
### 2. Какое утверждение относительно генерируемых колонок является ложью?
...
### 3. Имеется следующая структура, представленная на изображении. Было решено во все существующие записи в таблице role добавить пермиссию 'read' так, чтобы не было дубликатов. Как этого достичь?
...
### 4. Какой подход нужно использовать?
...
### 5. У вас есть две связанные таблицы, где t2 использует значение из последовательности по умолчанию при обновлении ссылочной записи. Какие проблемы могут возникнуть при удалении записей из t1?
...
### 6. В таблице some_table содержится 1 миллион записей. На колонке id создан индекс B-Tree, а на колонке name — индекс GIN для полнотекстового поиска. Какой запрос оптимально использует индекс, если выборка данных составляет менее 0.1% от общего объёма?
...
### 7. Таблица logs содержит миллиарды записей и используется для анализа активности. Наиболее частый запрос к таблице выбирает записи по колонке user_id и фильтрует их по диапазону значений в колонке event_time. Какой индекс лучше всего подходит для ускорения выполнения этого запроса?
...
### 8. Запускаются две транзакции Т1 и Т2. Какой вид изолированности транзакции НЕ позволит исполнить commit?
...
### 9. В PostgreSQL вы создаёте индекс в транзакции над таблицей users. Во время выполнения этой транзакции другая транзакция пытается выполнить одну из следующих команд над той же таблицей. Какая команда выполнится без конфликтов?
...
### 11. Какую команду следует использовать для тестирования на копии базы данных?
...
### 14. Следующая команда выполнена другим пользователем. Какой будет результат выполнения команды, если исполняющий пользователь не является суперпользователем и не является владельцем таблицы?
...
### 15. Какими методиками можно воспользоваться для поддержания работоспособности сервиса с учетом того, что запросов по ключу большинство?
...
### 16. Выберите корректный вывод по следующему запросу:
...
### 17. Какой из вариантов корректнее и безопаснее реализует задачу?

✅ **update customers set interests = array_append(interests, 'sports') where not 'sports' = any(interests)**
...
### 18. Почему этот запрос может работать медленно?
...
### 19. Какое влияние окажет добавление индекса на поле created_at в следующем запросе?
...
✅ **Ускорит выборку при большом объеме данных**
...
### 20. Что увидит транзакция T1 при уровне изоляции Read Committed?
...
✅ **Разные значения balance в двух SELECT**


### 21. Что может быть проблемой при сравнении чисел типа float?
...
✅ **0.1 может быть представлен неточно в памяти**

= работает только с целыми


### 22. Выберите верное утверждение о запросе:
...
### 23. Какой способ поможет оценить влияние каскадного удаления на производительность?
...
✅ **Выполнить EXPLAIN ANALYZE delete from users where id = 123**
...
### 24. Как убедиться, что триггер сработает корректно?
...
✅ **Попробовать обновить зарплату на меньшее значение и проверить, вызвана ли ошибка**
...
### 25. Почему возникает ошибка, если orders — это представление, и права на неё явно выданы?
...
✅ **У пользователя нет прав на таблицы, используемые внутри представления**
...
### 26. Вы хотите разделить таблицу users на два шарда по регионам. Какой шаг необходим при использовании PostgreSQL с Citus?

Переместить все таблицы в новую базу

Настроить шард-каталог вручную

✅ **Выполнить select create_distributed_table('users', 'region')**
...
### 27. Однако этот код не выполняется. Какую доработку следует внести, чтобы получить корректный результат?

○ select product, avg(price) from goods

✅ **select category, avg(price) from goods group by category**
...
### 28. Почему данный запрос может вернуть NULL в поле email?
...
✅ **Не все пользователи имеют записи в emails**
...
### 29. Почему в PostgreSQL по умолчанию индекс по полю deleted_at не помогает ускорить запрос:
...
✅ **B-Tree индекс не включает NULL-значения**
...
### 30. Какая доработка обеспечит устойчивое поведение при сохранении точности и отказоустойчивости?
...
✅ **Изменить тип поля в таблице на numeric(7,4)**


### 31. Что произойдёт при попытке вставить данные в представление?
...
o INSERT будет транслирован в базу через RULE

✅ **Ошибка: представление только для чтения**


### 32. Какой риск возникает, если пользователь создаёт выраженный индекс на поле, к которому у него нет прав?
...
✅ **Запрос завершится ошибкой из-за отсутствия доступа к полю**


### 33. Какой параметр нужно включить в postgresql.conf на реплике, чтобы разрешить подключение в режиме только чтения?
