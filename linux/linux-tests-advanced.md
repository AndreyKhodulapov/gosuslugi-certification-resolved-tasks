...
### 1. Как можно интерпретировать это предупреждение?
...
### 2. Какой командой в Linux можно отфильтровать в реальном времени только сообщения уровня ERROR из журналов systemd?
...
### 3. Какая ошибка в нём приводит к некорректной обработке файлов с пробелами в именах?
...
### 4. Вы хотите создать скрипт, который выводит по одному имени на строку всех файлов в текущем каталоге, начинающихся на file и заканчивающихся на .txt. Какой фрагмент кода нужно использовать?
...
### 5. Какая наиболее существенная проблема безопасности содержится в этой конфигурации?
...
### 6. Какое действие следует предпринять?
...
### 7. Что наиболее вероятно можно заключить по изменению состояния системы?
...
### 8. На сервере наблюдается замедленная работа приложений. Вам нужно в реальном времени определить, какие процессы загружают процессор и сколько оперативной памяти используется. Какую команду следует применить?
...
### 10. Что произойдёт, если сервер будет выключен в момент времени выполнения задачи (полночь) и будет включён в 08:00?
...
### 11. Какова наиболее вероятная причина этой проблемы?
...
### 13. Какой из вариантов лучше всего объясняет причину обнаруженной уязвимости?
...
### 14. Что из ниже перечисленного наиболее вероятно объясняет причину уязвимости?
...
### 15. Что означает эта запись для проверки файловой системы при загрузке?
...
### 16. Какой из следующих вариантов наилучшим образом объясняет ситуацию?
...
### 18. Скрипт читает список имён файлов из текстового файла filelist.txt, в котором строки могут содержать пробелы. Как безопасно обойти этот список в цикле?
...
### 20. Какая потенциальная уязвимость возникает в этой конфигурации?
...
✅ **Nginx может раскрывать произвольные файлы за пределами /uploads из-за некорректной обработки путей**


### 21. Вы не используете systemd и управляете Nginx напрямую. Как корректно инициировать перезапуск, чтобы все рабочие процессы были обновлены без прерывания клиентов?
...
✅ **Отправить master-процессу сигнал USR2, затем WINCH и TERM старому master**
...
### 22. Какое поведение системы можно считать потенциально проблемным в этом случае?

✅ **Физическая память почти полностью занята, активное использование swap указывает на нехватку ОЗУ**
...
### 23. Сервер начал потреблять до 100% CPU без явной активности со стороны прикладных процессов. Необходим инструмент, позволяющий в реальном времени отслеживать, какие функции ядра вызывают такую нагрузку. Что выбрать?
...
### 24. Что делает эта конфигурация?
...
✅ **Запускает скрипт в полночь ежедневно и полностью подавляет весь вывод**
...
### 25. Какой эффект оказывает RandomizedDelaySec в этом случае?
...
✅ **Запуск будет случайно сдвинут на время до 1 часа после запланированного события**

Таймер будет срабатывать каждый час после наступления daily


### 26. После замены традиционного network.service на NetworkManager.service сервер теряет сетевое соединение при перезагрузке. В чём может быть причина?

✅ **Служба не имеет соответствующей секции [Install]**
...
### 27. При отладке зависимости между юнитами systemd вы хотите визуализировать, какие службы блокируют или откладывают загрузку других. Что применить?
...
### 28. Приложение работает в домене sandbox, и доступ к системным файлам строго ограничен через SELinux. Администратор добавляет capability CAP_SYS_ADMIN к исполняемому файлу приложения. Что становится потенциальной угрозой безопасности?

В домене sandbox capability не учитываются, так как включён enforcing режим

✅ **CAP_SYS_ADMIN даёт широкие системные привилегии, включая монтирование файловых систем и изменение пространства имён, что может обойти SELinux изоляцию при наличии уязвимостей в ядре**
...
### 29. После монтирования часть содержимого /opt/base стала недоступна, а некоторые изменения не сохранились после перезагрузки. Что, вероятнее всего, пошло не так?

✅ **lowerdir был перекрыт overlay'ем, и базовая структура стала недоступна**
