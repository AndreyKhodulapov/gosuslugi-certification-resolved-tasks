...
**что произойдет при выполнении запроса select * from A inner join B om A.id = B.id where B.id is null?**

будут строки с null
будет ошибка
V **результат будет пустым**
будет одно совпадение
будут строки только из таблицы А


**В чем причина по которой avg(price) может вернуть null даже если price существует?**

avg не работает с типом string
V **все значения price null**
...
V **CREATE , TRUNCATE , DELETE , TRIGGER**
...
Запрос вернёт NULL
V **COUNT(price) не учитывает NULL, и результат может быть занижен**
...
V **При наличии WHERE-условия, отфильтровывающего строки из результата**
...
**Чем CASE отличается от IF в SQL?**

CASE работает только в WHERE
IF быстрее
IF нельзя использовать в SELECT
V **CASE — стандарт SQL, IF — диалектная конструкция**
...
V **Для изоляции агрегатной логики по каждой строке**


**Какой метод приведёт строку '2024-06-21 13:00:00' к дате без времени?**

V **Преобразовать строку в тип DATE, обрезающий время**
...
V **update orders set status = 'cancelled' where order_date < '2020-01-01'**
...
Как корректно отозвать право на удаление из таблицы orders у пользователя john?

REVOKE ON orders DELETE FOR john
DROP DELETE GRANT FROM john ON orders
REVOKE orders DELETE FROM john
V **REVOKE DELETE ON orders FROM john**
...
V **EXISTS проверяет наличие строк, ANY — сравнивает значения**


**Как изменить имя существующей таблицы в SQL?**

alter table users to new_name
rename table users by new_name
V **alter table users rename to new_name**
