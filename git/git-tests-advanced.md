...
### 1. Оцените, насколько правильно использовать Git в таком сценарии, и объясните, как это повлияет на производительность и размер репозитория в долгосрочной перспективе.
...
### 2. Вы с коллегой работаете над новой функциональностью в API в ветке feature/api-implementation. В ветку main было внесено несколько исправлений ошибок, которые также присутствуют в вашей ветке. Однако в main есть и другие изменения, несовместимые с вашей текущей веткой. Ваш коллега выполнил действия для исправления этих ошибок в вашей ветке, не затрагивая остальные функции.
...
✅ **git cherry-pick b2d83f4 c1e97f1 e3a91c2**
...
### 3. Какая команда была выполнена и какую команду теперь надо выполнить, чтобы восстановить ваши изменения?
...
✅ **Выполнена git reset --hard, которая удалила коммит a2f3143, а для восстановления надо выполнить git cherry-pick a2f3143**
...
### 4. Какой результат вы получите и почему?
...
### 5. Какой из следующих выводов отражает последствия таких действий с точки зрения расширенного Git Flow и управления историей изменений?
...
### 6. Какую команду надо выполнить для начала работы с git bisect?
...
### 8. Объясните, как технически работает Git LFS и какие преимущества он предоставляет по сравнению со стандартным Git при работе с большими файлами.
...
✅ **Git LFS заменяет большие файлы в репозитории на текстовые указатели (pointer files), содержащие SHA-256 хеш и метаданные, а сами файлы хранятся на отдельном LFS-сервере, это ускоряет операции клонирования и извлечения**
...
### 9. Какую операцию из перечисленных выполнил ваш коллега?
...
✅ **Выполнил git filter-branch --index-filter "git rm --cached debug.log" --all**

Выполнил git rebase -i для редактирования коммита и удаления файла


### 10. Какой из следующих подходов наиболее эффективен для интеграции внешнего репозитория, когда требуется сохранить полную историю изменений внешнего проекта и иметь возможность вносить локальные изменения, не затрагивая основной репозиторий?
...
### 11. Какая последовательность команд позволит восстановить потерянные коммиты и создать новую ветку с этими изменениями?
...
### 12. Какая последовательность команд Git корректно выполнит все эти требования?
...
✅ **it tag -a v2.5.0 a7d3f1c -m "Release v2.5.0" && git push origin v2.5.0**
...
### 13. Какая из последовательностей команд Git позволит выполнить все эти действия?
...
### 14. Какая стратегия будет эффективной для устранения уязвимости во всех ветках с минимальным риском нарушения работы проекта?
...
### 15. Какую команду в Git надо использовать, чтобы выполнить всё это в интерактивном режиме?
...
### 16. Разработчик вручную редактирует файлы в git/objects, чтобы восстановить поврежденные данные.

Оцените корректность такого подхода.

Это невозможно — файлы в директории git являются readonly

✅ **Это некорректно — такие действия могут привести к повреждению репозитория**
...
### 18. Что могло произойти?

✅ **Вы переключились на develop до того, как закоммитили изменения в feature/refactor-auth, поэтому merge не включил ожидаемые правки**
...
### 19. В процессе работы с git bisect вы столкнулись с коммитом, который невозможно проверить из-за отсутствия нужной среды. Что нужно сделать в этой ситуации?

Объявить коммит хорошим командой git bisect good

Объявить коммит плохим командой git bisect bad

✅ **Пропустить этот коммит командой git bisect skip**
...
### 20. Что необходимо сделать, чтобы клонировать репозиторий с подмодулями и сразу получить содержимое подмодулей?
...
✅ **Клонировать репозиторий с параметром —recurse-submodules**

Клонировать репозиторий и запустить git submodule add для каждого подмодуля


### 21. Команда решила удалить поддержку Git LFS из проекта. Какие шаги необходимы, чтобы безопасно удалить Git LFS и вернуть файлы в основной репозиторий?

Удалить .gitattributes и заново выполнить git add чтобы перезаписать файлы в истории

✅ **Выполнить git lfs migrate export --include="*" для замены pointer-файлов оригинальными файлами и удалить .gitattributes**
...
### 22. Какую операцию выполнила команда?

Выполнила git revert на коммите config.yaml

Выполнила git commit --amend и git push --force

✅ **Выполнила git filter-branch --index-filter "git rm --cached config.yaml" -- --all**
...
✅ **git reflog , найти хеш коммита, который был удален (он будет в reflog с его состоянием до начала rebase), затем git cherry-pick <хеш_потерянного_коммита>**
...
### 24. Какая команда Git позволит получить только этот конкретный тег из удаленного репозитория?
...
✅ **git fetch origin tag test-build-1**
...
### 25. Какой способ подходит для этого случая?

Выполнить git rebase -i и удалить указанные коммиты — затем форсировать пуш

✅ **Выполнить git revert 7a5c4f1..9fcb1d3 — создаст обратные коммиты для всей группы**
...
### 26. Вы хотите, чтобы Second commit стал последним. Как должен выглядеть новый порядок?
