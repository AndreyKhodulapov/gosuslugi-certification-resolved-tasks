ВНИМАНИЕ!!! ЭТИ ТЕСТЫ НА ДАННЫЙ МОМЕНТ ПРОСТО ПРОПУЩЕНЫ ЧЕРЕЗ НЕЙРОНКУ
ЗА ПРАВИЛЬНОСТЬ НЕ РУЧАЮСЬ! КАК ПРОВЕРЮ НА НН ЭТА ЗАПИСЬ ИЗМЕНИТСЯ - НАПИШУ СКОЛЬКО ЗДЕСЬ ЕСТЬ ОШИБОК.
...
V **[[m[j][i] for j in range(len(m))] for i in range(len(m[j]))]**
...
V **Переменная x примет значение 6** (но такого ответа нет)
...
V **Многопоточность позволяет выполнять несколько потоков одновременно, тогда как асинхронность использует один поток для выполнения нескольких задач в период завершения.**
