## 19.10 2025 HIT RATE = 37%; SCORE =  завалили за нейронку;

ПРАВИЛЬНЫЙ ОТВЕТ ОБОЗНАЧАЕТСЯ ГАЛОЧКОЙ ✅ **и жирным шрифтом**

### Вопрос 1. Для какой пары встроенных типов данных попытка применения операции '+' НЕ приведёт к ошибке?
...
### Вопрос 2. Что будет выведено в терминал в результате выполнения следующего кода?
...
✅ **0011** - вообще вывод вот такой возможно условие переписано неверно


### Вопрос 3. Что будет выведено в терминал в результате выполнения следующего кода?
...
### Вопрос 4. Что будет выведено в терминал в результате выполнения следующего кода?
...
### Вопрос 5. Что произойдёт при попытке выполнения следующего кода?
...
### Вопрос 6. Какой генератор списков транспонирует (заменяет строки на столбцы) матрицу m?

[[m[j][i] for j in range(len(m[i]))] for i in range(len(m))]

✅ **[[m[j][i] for j in range(len(m))] for i in range(len(m))]**
...
### Вопрос 7. Выберите вариант, НЕВЕРНО перечисляющий встроенные модули
...
### Вопрос 8. В чём разница между использованием with и open() close() при работе с файлами?
...
### Вопрос 9. Какая строка целиком соответствует следующему регулярному выражению?
...
✅ **'123"456'** самый близкий вариант, правильного нет возможна ощибка условия


### Вопрос 10. Какие аргументы можно передать в map()?
...
### Вопрос 11. В каком случае требуется использовать пользовательские типы ошибок?
...
### Вопрос 12. Что произойдёт при выполнении следующего кода?
...
### Вопрос 14. Какое значение примет переменная result после выполнения следующего кода?
...
### Вопрос 15. Выберите НЕВЕРНОЕ утверждение о модели памяти Python.
...
### Вопрос 16. Выберите НЕВЕРНОЕ утверждение.
...
### Вопрос 17. Что приводит к ошибке при попытке выполнения следующего кода?
...
### Вопрос 18. Функция primes(ps) предназначена для возврата списка из простых чисел, начиная с 2 или с числа, следующего за последним в списке ps, если он передан. Какой из предложенных вариантов устраняет проблему?
...
### Вопрос 19. Что произойдёт при попытке выполнения следующего кода?
...
✅ **Будет вызван метод класса N**  (потом код упадет с ошибкой но НН на это похер)
...
### Вопрос 20. Какой вариант возвращает список, который содержит элементы исключительно под главной диагональю (слева направо) квадратной матрицы m?
...
### Вопрос 21. Выберите вариант, НЕВЕРНО перечисляющий встроенные модули
...
### Вопрос 22. Какое из следующих утверждений верно?
...
### Вопрос 23. Какое регулярное выражение полностью соответствует предложению, которое состоит из разделённых пробелами слов и заканчивается точкой?
...
### Вопрос 24. Какие аргументы можно передать в map()?
...
### Вопрос 25. Что будет выведено в терминал по результатам выполнения кода?
...
### Вопрос 26. Выберите НЕВЕРНОЕ утверждение.
...
### Вопрос 27. Что произойдёт в результате выполнения следующего кода?
...
✅ **Переменная x примет значение 6** (но такого ответа нет)


### Вопрос 28. Выберите НЕВЕРНОЕ утверждение об операторе del.
...
### Вопрос 29. Какие из индексов results будут содержать True после выполнения следующего кода?
...
### Вопрос 31. Какие числа будут выведены в терминал в результате выполнения следующего кода?
...
### Вопрос 32. Что будет выведено в терминал в результате выполнения следующего кода?
...
### Вопрос 32. Какой из перечисленных модулей содержит функцию, которая фильтрует элементы одного списка по элементам другого?
...
### Вопрос 33. Выберите некорректный режим открытия файлов
...
### Вопрос 34. Какие строки будут обнаружены findall() при выполнении следующего кода?
...
### Вопрос 35. Что будет выведено в терминал в результате выполнения следующего кода?
...
### Вопрос 36. Что произойдет при выполнении следующего кода?
...
✅ **5.0, Успещно выполнено, Завершено**
...
### Вопрос 37. Какой список будет выведен в терминал в результате выполнения следующего кода?
...
✅ **Возникнет ошибка, связанная с неправильным числом аргументов**
...
### Вопрос 38. Что произойдёт в результате выполнения следующего кода?
...
✅ **Будет вызван метод f класса n**

### Вопрос 39. Какая из строк НЕ соответствует этому регулярному выражению?
...
lambda может использоваться для фиксации аргументов функции

lambda поддерживает значения аргумента по умолчанию

Функции, объявленные при помощи lambda и def, имеют один и тот же тип

✅ **Вызов filter(x,y) эквивалентен [x[index] for index in range(x) if y[index]]**

Функция map() не возвращает список


### Вопрос 41. Выберите верное утверждение о механизме обработки ошибок.
...
✅ **Блок try может содержать несколько блоков except, но для одного исключения будет выполнен только один**

Нельзя иметь блок try без блока except


### Вопрос 42. Выберите НЕВЕРНОЕ утверждение о модели памяти Python.

Python не поддерживает указатели

В Python может произойти утечка памяти

✅ **В Python не допускаются циклические ссылки**
...
### Вопрос 42. Выберите НЕВЕРНОЕ утверждение о многопоточности в Python.

✅ **Основная программа никогда не ждёт окончания исполнения потока**
